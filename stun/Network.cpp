...
struct in_addr resolveHostName(const std::string& name)
{
    ...
    // printf("IP : %s\n", inet_ntoa(ip));
    //  ulong l1 = inet_addr("9.135.231.182");
    // memcpy(&ip, &l1, 4);
    // return ip;
    // ip = inet_addr("9.135.231.182");
    return ip;
}
...
UdpSocket::UdpSocket()
{
    ...
    cliaddr.sin_port = htonl(11780);

    int ret = ::bind(_socket, (struct sockaddr*)&cliaddr, sizeof(cliaddr));
    assert(ret == 0);
}
...
sockaddr_in UdpSocket::localAddress()
{
    ...
    {
        LOG(ERROR) << "UdpSocket::localAddress() failed!";
    }
    return sin;
}
...
ssize_t UdpSocket::writeTo(const char* buf, size_t size, std::string host, unsigned int port)
{
    sockaddr_in sin;
    sin.sin_family = AF_INET;
    sin.sin_port = htons(port);
    sin.sin_addr.s_addr = resolveHostName(host).s_addr;
    return ::sendto(_socket, buf, size, 0, (struct sockaddr*)&sin, sizeof(sin));
}
